...
class EvaluationResult:
    ...
    known_algorithm: KnownAlgorithmResult | None = None
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        return {
            ...
            "known_algorithm": {
                "is_known": self.known_algorithm.is_known if self.known_algorithm else False,
                "algorithm_name": self.known_algorithm.algorithm_name if self.known_algorithm else None,
                "confidence": self.known_algorithm.confidence if self.known_algorithm else 0.0,
                "penalty_applied": self.known_algorithm.penalty_applied if self.known_algorithm else 0.0,
            },
...
class UniversalEvaluator:
    ...
    4. Known Algorithm Detection - penalizes known solutions
    ...
    def __init__(
        ...
        # Initialize known algorithm detector
        self._known_algorithm_detector = self._init_detector()

    def _init_detector(self):
        """Initialize known algorithm detector based on config."""
        from obsidian.research.known_algorithms import (
            KnownAlgorithmDetector,
            create_detector_from_definitions,
        )
        ...
        # Prefer dynamic definitions (generated by Claude during research-init)
        if config.definitions:
            return create_detector_from_definitions(config.definitions)

        # Legacy fallback: use built-in database
        if config.algorithms:
            ...
            return detector if detector.algorithms else None
    ...
    def evaluate(
        ...
        # 4. KNOWN ALGORITHM DETECTION
        known_algorithm = self._detect_known_algorithm(
            solution_path, benchmark_result=benchmark
        )

        # 5. EXTRACT NICHE VALUES
        ...
        # Apply known algorithm penalty
        score = self._apply_known_algorithm_penalty(base_score, known_algorithm)
    ...
    def _detect_known_algorithm(
        self,
        solution_path: Path,
        benchmark_result: BenchmarkResult | None = None,
    ) -> KnownAlgorithmResult:
        """Detect if solution matches a known algorithm."""
        if self._known_algorithm_detector is None:
            return KnownAlgorithmResult(is_known=False)

        code = solution_path.read_text()

        # Extract behavioral data from benchmark if available
        behavioral_data = None
        if benchmark_result and benchmark_result.details:
            behavioral_data = benchmark_result.details

        # Also try to parse behavioral data from benchmark output
        if benchmark_result and benchmark_result.output:
            try:
                import json
                parsed = json.loads(benchmark_result.output.strip())
                if isinstance(parsed, dict):
                    behavioral_data = parsed
            except (json.JSONDecodeError, ValueError):
                pass

        config = self.problem.novelty.known_algorithms
        result = self._known_algorithm_detector.detect(
            code,
            behavioral_data,
            confidence_threshold=config.confidence_threshold,
        )

        return KnownAlgorithmResult(
            is_known=result.is_known,
            algorithm_name=result.algorithm_name,
            confidence=result.confidence,
            penalty_applied=result.penalty if result.is_known else 0.0,
            details=result.match_details,
        )

    def _apply_known_algorithm_penalty(
        self,
        base_score: float,
        known_result: KnownAlgorithmResult,
    ) -> float:
        """Apply penalty for known algorithms."""
        if not known_result.is_known and known_result.penalty_applied == 0:
            return base_score

        config = self.problem.novelty.known_algorithms
        penalty = known_result.penalty_applied

        if config.penalty_mode == "multiplicative":
            # Score * (1 - penalty)
            # E.g., 0.9 penalty -> score * 0.1
            return base_score * (1.0 - penalty)
        else:
            # Additive: score - penalty
