...
class AlgorithmDefinition:
    """Definition of a known algorithm (generated by Claude during research-init)."""

    name: str
    description: str = ""
    penalty: float = 0.8  # 0.0 to 1.0
    keywords: list[str] = field(default_factory=list)
    patterns: list[str] = field(default_factory=list)  # Regex patterns


@dataclass
class KnownAlgorithmConfig:
    """Configuration for known algorithm detection."""

    enabled: bool = True
    definitions: list[AlgorithmDefinition] = field(default_factory=list)  # Dynamic definitions
    algorithms: list[str] = field(default_factory=list)  # Legacy: names from built-in db
    penalty_mode: str = "multiplicative"  # "multiplicative" or "additive"
...
class NoveltyConfig:
    ...
    known_algorithms: KnownAlgorithmConfig = field(default_factory=KnownAlgorithmConfig)
...
def _parse_algorithm_definitions(data: list[dict[str, Any]]) -> list[AlgorithmDefinition]:
    """Parse algorithm definitions from YAML."""
    definitions = []
    for item in data:
        definitions.append(
            AlgorithmDefinition(
...
def _parse_known_algorithms(data: dict[str, Any]) -> KnownAlgorithmConfig:
    """Parse known algorithm detection configuration."""
    if not data:
        return KnownAlgorithmConfig()

    # Parse inline definitions (preferred - generated by Claude)
    definitions = _parse_algorithm_definitions(data.get("definitions", []))

    return KnownAlgorithmConfig(
        enabled=data.get("enabled", True),
        definitions=definitions,
        algorithms=data.get("algorithms", []),  # Legacy fallback
        penalty_mode=data.get("penalty_mode", "multiplicative"),
        confidence_threshold=data.get("confidence_threshold", 0.6),
        variation_penalty_factor=data.get("variation_penalty_factor", 0.5),
    )
...
def _parse_novelty(data: dict[str, Any]) -> NoveltyConfig:
    ...
    return NoveltyConfig(
        ...
        known_algorithms=_parse_known_algorithms(data.get("known_algorithms", {})),
    )
